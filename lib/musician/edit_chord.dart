...
class EditChord extends StatefulWidget {
  final String songId;
  final String title;
  ...
  @override
  State<EditChord> createState() => _EditChordState();
}

class _EditChordState extends State<EditChord>
  ...
  Future<void> fetchSongDetails() async {
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        setState(() {
          // .text hmang in thun ding, controller thar create lo ding
          titleController.text = data['title'] ?? '';
          singerController.text = data['singer'] ?? '';
          chordsController.text = data['chords'] ?? '';
          ytController.text = data['ytLink'] ?? '';
          selectedKey = data['key'] ?? 'C';
          status = data['status'] ?? '';
          type = data['type'] ?? '';
          uid = data['uploaderUid'] ?? '';
          email = data['uploaderEmail'] ?? '';
          isLoading = false; // Loading a dih cang tiah thlen chih
        });
      }
    } catch (e) {
  ...
  Future<void> saveChordToFirebase() async {
    ...
    try {
      // 1. A um cia mi songId hmang in UPDATE tuah
      await FirebaseFirestore.instance.collection('musicianChords').doc(widget.songId).update({
        'title': titleController.text.trim(),
        'singer': singerController.text.trim(),
        'chords': chordsController.text.trim(),
        'key': selectedKey,
        'ytLink': ytController.text.trim(),
        'last_edited_at': FieldValue.serverTimestamp(), // Edit lio caan
      });

      // 2. Metadata (Search Index) zong update a hau
      ...
      await metaRef.set({
        'songs_map': {
          widget.songId: {
            'title': titleController.text.trim(),
            'singer': singerController.text.trim(),
            // uploaderUid le a dang cu thlen a hau lo
          }
        }
  ...
  void addText(String input) {
    ...
    if (input == 'x' || input == 'Clear') {
      ...
      final spaceNeeded = RegExp(r'^([A-G][#b♭]?[a-zA-Z0-9]*|_)$').hasMatch(input);

      String textToInsert;

      if (spaceNeeded) {
        final isAtStart = cursorPos == 0;
        final endsWithSpace = text.isNotEmpty && text[cursorPos - 1] == ' ';

        textToInsert = (isAtStart || endsWithSpace) ? input : ' $input';
      } else {
        textToInsert = input;
      }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        onChanged: (value) => setState(() => chordsController.text = value),
